...
public class ItemControllerTest {
    ...
    @Test
    public void addItemWithNullNameBadTest() throws Exception {
        ItemDto itemDto = new ItemDto("item", "description", true);
        itemDto.setId(1);
        itemDto.setName(null);
        long userId = 1;
        when(itemService.addItem(itemDto, userId)).thenReturn(itemDto);
        mvc.perform(post("/items").header("X-Sharer-User-Id", userId).content(mapper.writeValueAsString(itemDto)).characterEncoding(StandardCharsets.UTF_8).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)).andExpect(status().isBadRequest());
        verify(itemService, times(0)).addItem(itemDto, userId);

    }

    @Test
    public void addItemWithNotBlankNameBadTest() throws Exception {
        ItemDto itemDto = new ItemDto("item", "description", true);
        itemDto.setId(1);
        itemDto.setName("");
        long userId = 1;
    ...
    @Test
    public void addItemWithNullDescriptionBadTest() throws Exception {
        ItemDto itemDto = new ItemDto("item", "description", true);
        itemDto.setId(1);
    ...
    @Test
    public void addItemWithNotBlankDescriptionBadTest() throws Exception {
        ItemDto itemDto = new ItemDto("item", "description", true);
        itemDto.setId(1);
    ...
    @Test
    public void addItemWithNullAvailableBadTest() throws Exception {
        ItemDto itemDto = new ItemDto("item", "description", true);
        itemDto.setId(1);
    ...
    @Test
    public void searchItemsGoodTest() throws Exception {
        String text = "qwerty";
        List<ItemDto> itemDtos = new ArrayList<>();
        itemDtos.add(new ItemDto(1, "item", "description", true));
        itemDtos.add(new ItemDto(2, "new Item", " fwf", false));
        when(itemService.searchItems(text, 0, 2)).thenReturn(itemDtos);
        mvc.perform(get("/items/search").queryParam("from", "0").queryParam("size", "2").queryParam("text", "qwerty")).andExpect(status().isOk()).andExpect(jsonPath("$.[0].id", is(itemDtos.get(0).getId()), Long.class)).andExpect(jsonPath("$.[0].name", is(itemDtos.get(0).getName()), String.class)).andExpect(jsonPath("$.[0].description", is(itemDtos.get(0).getDescription()), String.class)).andExpect(jsonPath("$.[0].available", is(itemDtos.get(0).getAvailable()), Boolean.class)).andExpect(jsonPath("$.[1].id", is(itemDtos.get(1).getId()), Long.class)).andExpect(jsonPath("$.[1].name", is(itemDtos.get(1).getName()), String.class)).andExpect(jsonPath("$.[1].description", is(itemDtos.get(1).getDescription()), String.class)).andExpect(jsonPath("$.[1].available", is(itemDtos.get(1).getAvailable()), Boolean.class));
        verify(itemService, times(1)).searchItems(text, 0, 2);
